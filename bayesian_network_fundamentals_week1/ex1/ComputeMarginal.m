...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % 1. Eliminate any data contrary to observed evidence
  F = ObserveEvidence(F, E);
  % 2. Compute the joint distribution of all factors
  M = ComputeJointDistribution(F);
  % 3. Marginalize away all variables except those in V
  %    Returned PDF may not have vars in same order as V
  M = FactorMarginalization(M, setdiff(M.var, V));
  % 4. Normalize probability returned.
